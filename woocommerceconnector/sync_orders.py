...
def _safe_error_msg(e):
    """Normalize exception message to str safely (handles bytes/str)."""
    msg = e.args[0] if (hasattr(e, "args") and e.args) else ""
...
def sync_woocommerce_orders():
    ...
    for woocommerce_order_status in woocommerce_order_status_for_import:
        for woocommerce_order in get_woocommerce_orders(woocommerce_order_status):
            # validate that woocommerce order has line items
            ...
            if not so:
                if valid_customer_and_product(woocommerce_order):
                    try:
                        ...
                        # SAFE: normalize error message (no .decode on str in py3)
                        msg = _safe_error_msg(e)
                        status = getattr(e, "status_code", None) or getattr(e, "http_status", None)
                        if status == 402 or msg.startswith("402"):
                            raise e
                        else:
                            make_woocommerce_log(
                                title=str(e),
...
def valid_customer_and_product(woocommerce_order):
    ...
    if customer_id == 0:  # we are dealing with a guest customer
        if not frappe.db.get_value(
            "Customer",
            {"woocommerce_customer_id": "Guest of Order-ID: {0}".format(woocommerce_order.get("id"))},
            "name",
            False,
            True
        ):
...
def _link_address_to_customer(address_name, customer_name):
    """Ensure Address doc is dynamically linked to the given Customer."""
    if not address_name:
        return
    addr = frappe.get_doc("Address", address_name)
    links = {(l.link_doctype, l.link_name) for l in addr.links}
    if ("Customer", customer_name) not in links:
...
def create_sales_order(woocommerce_order, woocommerce_settings, company=None):
    ...
    if not so:
        ...
        # Debug helpful for tracing address values
        frappe.logger().info({
            "woocommerce_debug": "SO addresses",
            "customer": so.customer,
            "billing_address": so.customer_address,
            "shipping_address": so.shipping_address_name,
        })

        # Ensure addresses are linked to the same Customer before saving
        _link_address_to_customer(so.customer_address, so.customer)
        _link_address_to_customer(so.shipping_address_name, so.customer)
...
def get_customer_address_from_order(type, woocommerce_order, customer):
    ...
    address_name = frappe.db.get_value(
        "Address",
        {"woocommerce_address_id": type, "address_line1": address_record.get("address_1"), "woocommerce_company_name": address_record.get("company") or ''},
        "name"
    )
    if not address_name:
        ...
        if not frappe.db.exists("Country", country):
            country = "Switzerland"
        try:
            address_name_doc = frappe.get_doc({
                "doctype": "Address",
            ...
            address_name = address_name_doc.name
            ...
            make_woocommerce_log(
                ...
                message=frappe.get_traceback(),
                request_data={"address_record": address_record, "customer": customer},
                exception=True
            )
...
def get_shipping_account_head(shipping):
    shipping_title = shipping.get("method_title") or ""
...
def get_tax_account_head(tax):
    tax_title = tax.get("name") or tax.get("method_title") or ""
